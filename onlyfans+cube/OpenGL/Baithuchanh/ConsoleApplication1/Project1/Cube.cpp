...
int main(void)
{
	...
	GLfloat vertices[] = {
		// vị trí - position         //texture coordinate (s,t)(u,v)
		   //x       y     z		R G  B					 u     v
		   //Behind-Back(Z-)(XY)	Color			(left) (down)
			...
			-0.5f, -0.5f, -0.5f,  0.3f,0.5f,0.0f,	//0.0f, 1.0f,
			 0.5f, -0.5f, -0.5f,  0.3f,0.5f,0.0f,	//1.0f, 1.0f,
			 0.5f, -0.5f,  0.5f,  0.3f,0.5f,0.0f,	//1.0f, 0.0f,
			 0.5f, -0.5f,  0.5f,  0.3f,0.5f,0.0f,	//1.0f, 0.0f,
			-0.5f, -0.5f,  0.5f,  0.3f,0.5f,0.0f,	//0.0f, 0.0f,
			-0.5f, -0.5f, -0.5f,  0.3f,0.5f,0.0f,	//0.0f, 1.0f,
...
//unbind VAO
	...
	unsigned char* image = stbi_load("black_cat_texture_2.png", &width, &height, &nrChannels, 0);
	if (image)
	{
		///tạo mipmaps
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate(model, (float)glfwGetTime() / 2.0f, glm::vec3(0.0f, 1.0f, 0.0f));//muốn xoay mô hình đối tượng 1 góc 30 9theo trục x
		view = glm::translate(view, glm::vec3(0.0f, 0.0f, -1.0f));
		projection = glm::perspective(130.0f, (float)WIDTH / (float)HEIGHT, 0.1f, 100.0f);
		///xác định ma trận biến đổi (các bạn muốn biến đổi đối tượng như nào )
	//glm::mat4 Trans = glm::mat4(1.0f);
	//dịch chuyển theo x,y,z lần lượt là 0.5f,-0.5f,0.0f
	//Trans = glm::translate(Trans, glm::vec3(0.3f, 0.3f, 0.0f)); ///Trans=Trans*glm::vec3(0.0f, -0.5f,1.0f)
	...
	//lấy vị trị của uniform
		...
		//Trans = glm::scale(Trans, glm::vec3(2.0f, 2.0f, 2.0f)); ///Trans=translate*Trans
			// roate 60 độ quanh trục y
		//Trans = glm::rotate(Trans, (float)glfwGetTime()/ 1.0f, glm::vec3(0.0f, 1.0f, 0.0f)); ///Trans=translate*Trans
		...
		glBindVertexArray(VAO);
		//model = glm::translate(view, List_CubePosition[0]);
		model = glm::translate(view, glm::vec3(0.0f, 0.0f, -1.0f));

		//float angle = 20.0f;
		//model = glm::rotate(model, glm::radians(angle), glm::vec3(1.0f, 0.3f, 0.5f));
		model = glm::rotate(model, (float)glfwGetTime() / 1.0f, glm::vec3(1.0f, 0.5f, 0.5f));
